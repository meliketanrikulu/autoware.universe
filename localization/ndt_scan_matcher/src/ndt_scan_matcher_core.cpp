        ...
        geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr pose_conv_msg_ptr)
{
    ...
    trustedPose.pose_avarage_rmse = (std::sqrt(trusted_source_pose_.pose.covariance[0]) + std::sqrt(trusted_source_pose_.pose.covariance[7])) / 2;
...
std::array<double, 36> NDTScanMatcher::covariance_modifier(std::array<double, 36> & in_ndt_covariance){
    ...
    if(trustedPose.pose_avarage_rmse <= 0.10 && trustedPose.yaw_rmse < 0.025){
        close_ndt_pose_source_ = true;
    }
    else if(trustedPose.pose_avarage_rmse <= 0.10){
        ndt_covariance[35] = ndt_covariance[35] * 1000;
    }
    else if(trustedPose.pose_avarage_rmse <= 0.25){
