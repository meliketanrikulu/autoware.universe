...
void AWPoseCovarianceModifierNode::trusted_pose_with_cov_callback(const geometry_msgs::msg::PoseWithCovarianceStamped::ConstSharedPtr &msg) {
    ...
    if (trusted_pose_rmse_ > 0.25){
        ...
        if (trusted_pose_yaw_rmse_in_degrees_ >= 0.025){
            pose_estimator_pose.pose.covariance[35] = pose_estimator_pose.pose.covariance[35] * 1000;
        }

        pose_estimator_pose.pose.pose.position.z = msg->pose.pose.position.z - 0.4;
